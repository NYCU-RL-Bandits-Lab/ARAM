...
class SimulatedNetworkEnv(gym.Env):
    def __init__(self,
        ...
        self.action_space = spaces.Box(np.array([-1e1, -1e1, -1e1, -1e1, -1e1, -1e1, -1e1, -1e1, -1e1]), np.array([1e1, 1e1, 1e1, 1e1, 1e1, 1e1, 1e1, 1e1, 1e1]), dtype=np.float32)
    ...
    def step(self, actions):
        ...
        if Sender0_mode == 0:
            ...
            actions = 50 * (np.array(actions) + 1 + 1e-6)
            """
...
register(id='PccNs_uti_NSFnet_multiV2-v0', entry_point='environments.NSFnet.NSFnet_multiV2:SimulatedNetworkEnv')
